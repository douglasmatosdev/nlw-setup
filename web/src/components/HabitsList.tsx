...
interface HabitsListProps {
  date: Date;
  completedChanged: (completed: number) => void;
}
...
export function HabitsList({ date, completedChanged }: HabitsListProps) {
  ...
  async function handleToggleHabit(habitId: string) {
    ...
    completedChanged(completedHabits.length);
  }
      ...
      {habitsInfo?.possibleHabits.map((habit) => {
        return (
          <Checkbox.Root
            ...
            className="flex items-center gap-3 group outline-none transition-colors focus:outline-none disabled:cursor-not-allowed"
          >
